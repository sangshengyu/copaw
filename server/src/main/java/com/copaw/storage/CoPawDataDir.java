...
/**
 * Utility class for resolving CoPaw data directory paths.
 *
 * <p>This class provides centralized access to various CoPaw data directories
 * based on the configured data directory (default: ~/.copaw).</p>
 */
@Component
public class CoPawDataDir {

    private final Path dataDir;
    private final Path secretDir;

    public CoPawDataDir(
            @Value("${copaw.data-dir:#{null}}") String configuredDataDir,
            @Value("${copaw.secret-dir:#{null}}") String configuredSecretDir) {
        // Resolve data directory
        if (configuredDataDir != null && !configuredDataDir.isEmpty()) {
            this.dataDir = resolvePath(configuredDataDir);
        } else {
            this.dataDir = Paths.get(System.getProperty("user.home"), ".copaw");
        }

...
/**
 * Agent Manager - manages multiple SAAgentEngine instances.
 * 
 * Responsibilities:
...
public class AgentManager {
    ...
    private final Map<String, SAAgentEngine> engines = new ConcurrentHashMap<>();
     ...
     */
    public SAAgentEngine getEngine(String agentId) {
     ...
     */
    private SAAgentEngine createEngine(String agentId) {
        log.info("Creating engine for agent: {}", agentId);
        
        AgentProfileConfig config = agentService.getAgentConfig(agentId);
        
        return new SAAgentEngine(config, providerService, toolManager);
    }
     ...
     */
    public SAAgentEngine reloadEngine(String agentId) {
        log.info("Reloading engine for agent: {}", agentId);
        
        // Stop existing engine if running
        SAAgentEngine existing = engines.get(agentId);
    ...
    public boolean isRunning(String agentId) {
        SAAgentEngine engine = engines.get(agentId);
    ...
    public void shutdown() {
        ...
        for (Map.Entry<String, SAAgentEngine> entry : engines.entrySet()) {
            try {

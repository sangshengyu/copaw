...
from sa.__version__ import __version__
from sa.cli.main import cli
from sa.cli.update_cmd import (
    InstallInfo,
    RunningServiceInfo,
...
def _install_info(
    *,
    ...
    return InstallInfo(
        package_dir="/tmp/site-packages/sa",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "url": "file:///Users/test/SolutionArchitect",
                "dir_info": {"editable": True},
            },
            ("editable", "file:///Users/test/SolutionArchitect"),
        ),
        (
            {
                "url": "https://github.com/agentscope-ai/SolutionArchitect.git",
                "vcs_info": {"vcs": "git", "commit_id": "abc123"},
            },
            ("vcs", "https://github.com/agentscope-ai/SolutionArchitect.git"),
        ),
        (
            {"url": "file:///tmp/sa.whl"},
            ("local", "file:///tmp/sa.whl"),
        ),
        (
            {"url": "https://example.com/sa.whl"},
            ("direct-url", "https://example.com/sa.whl"),
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "uv",
            "editable",
            "file:///Users/test/SolutionArchitect",
        ),
    ],
)
def test_detect_installation(
    ...
    from sa.cli import update_cmd as update_cmd_module
...
def test_update_reports_up_to_date(monkeypatch) -> None:
    ...
    assert "SolutionArchitect is already up to date." in result.output
...
def test_probe_service_ignores_proxy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("sa.cli.update_cmd.httpx.get", _fake_get)
...
def test_update_blocks_running_service(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "Please stop it before running `sa update`" in result.output
    assert (
        "without `--yes` to confirm a forced `sa shutdown`" in result.output
    )
...
def test_update_can_cancel_forced_shutdown(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert (
        "forcibly terminate the current SolutionArchitect backend/frontend "
...
def test_update_can_force_shutdown_running_service(
    ...
    def _fake_shutdown(
        ...
        class _Result:
            returncode = 0
            stdout = "Stopped SolutionArchitect processes: 1234\n"
    ...
    assert result.exit_code == 0
    assert "Running `sa shutdown` before updating..." in result.output
    assert "Stopped SolutionArchitect processes: 1234" in result.output
    assert "Starting SolutionArchitect update..." in result.output
...
def test_update_worker_waits_for_launcher_exit(
    ...
    monkeypatch.setattr(
        "sa.cli.update_cmd._wait_for_process_exit",
...
def test_run_update_worker_detached_spawns_without_capture(
    ...
    monkeypatch.setattr(
        "sa.cli.update_cmd._spawn_update_worker",
        _fake_spawn,
    )
...
def test_update_worker_foreground_streams_output_and_cleans_plan(
    ...
    assert return_code == 0
    assert "[sa] Updating SolutionArchitect 1.0.0 -> 9.9.9..." in captured.out
    assert "[sa] Using installer: integration-test" in captured.out
    assert "installer: preparing" in captured.out
    assert "installer: done" in captured.out
    assert "[sa] Update completed successfully." in captured.out
...
def test_update_worker_foreground_propagates_failure_exit_code(
    ...
    assert "[sa] Update failed with exit code 7." in captured.out
    assert not plan_path.exists()

# Contributing to SolutionArchitect

## Welcome! 🐾

Thank you for your interest in contributing to SolutionArchitect! SolutionArchitect is an open-source **personal AI assistant** that runs in your own environment—on your machine or in the cloud. It connects to DingTalk, Feishu, QQ, Discord, iMessage, and other chat apps, supports scheduled tasks and heartbeat, and extends its capabilities through **Skills**. We warmly welcome contributions that help make SolutionArchitect more useful for everyone: whether you add a new channel, a new model provider, a Skill, improve docs, or fix bugs.

**Quick links:** [GitHub](https://github.com/agentscope-ai/SolutionArchitect) · [Docs](https://sa.agentscope.io/) · [License: Apache 2.0](LICENSE)
...
- **Check [Open Issues](https://github.com/agentscope-ai/SolutionArchitect/issues)** and any [Projects](https://github.com/agentscope-ai/SolutionArchitect/projects) or roadmap labels.
...
SolutionArchitect is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

SolutionArchitect supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/sa/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in SolutionArchitect.
...
Channels are how SolutionArchitect talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so SolutionArchitect can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/sa/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/sa/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `sa channels install <key>` — create a template or copy from `--path` / `--url`
  - `sa channels add <key>` — install and add to config
  - `sa channels remove <key>` — remove custom channel from `custom_channels/`
  - `sa channels config` — interactive config
...
**Skills** define what SolutionArchitect can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/sa/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** SolutionArchitect supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
SolutionArchitect aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `sa init` / `sa app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **MCP (Model Context Protocol):** SolutionArchitect supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
- **Documentation:** Fixes and improvements to [the docs](https://sa.agentscope.io/) (under `website/public/docs/`) and README are always welcome.
...
- **Discussions:** [GitHub Discussions](https://github.com/agentscope-ai/SolutionArchitect/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/agentscope-ai/SolutionArchitect/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to SolutionArchitect. Your work helps make it a better assistant for everyone. 🐾

# -*- coding: utf-8 -*-
"""
Skill security scanner for SolutionArchitect.
...
Quick start::
    ...
    from sa.security.skill_scanner import SkillScanner
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``SOLUTION_ARCHITECT_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = os.environ.get("SOLUTION_ARCHITECT_SKILL_SCAN_MODE")
    if env is not None:
        val = env.lower().strip()
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        return Path.home() / ".sa" / _BLOCKED_HISTORY_FILE

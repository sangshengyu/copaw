# -*- coding: utf-8 -*-
"""SolutionArchitect Agent - Main agent implementation.

This module provides the main SAAgent class built on ReActAgent,
...
class SAAgent(ToolGuardMixin, ReActAgent):
    """SolutionArchitect Agent with integrated tools, skills, and memory management.
    ...
    Python's MRO: SAAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_sa_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_sa_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += SAAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append({"type": "text", "text": SAAgent._ROUND_END_NOTICE})
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        logger.info("SAAgent.reply: max_iters=%s", self.max_iters)

# -*- coding: utf-8 -*-
"""Token counting utilities for SolutionArchitect using HuggingFace tokenizers.
...
class SATokenCounter(HuggingFaceTokenCounter):
    """Token counter for SolutionArchitect with configurable tokenizer support.
...
class SAEstimateTokenCounter(HuggingFaceTokenCounter):
...
_token_counter_cache: dict[
    tuple,
    SATokenCounter | SAEstimateTokenCounter,
] = {}


def get_sa_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a SAEstimateTokenCounter
            that uses only character-based estimation without loading a
            tokenizer. If False, returns a full SATokenCounter backed
    ...
    if use_estimate:
        ...
        if config_key not in _token_counter_cache:
            _token_counter_cache[config_key] = SATokenCounter(

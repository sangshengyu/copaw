---
name: guidance
description: "回答用户关于 SolutionArchitect 安装与配置的问题：优先定位并阅读本地文档，再提炼答案；若本地信息不足，兜底访问官网文档。"
metadata:
  {
...
# SolutionArchitect 安装与配置问答指南

当用户询问 **SolutionArchitect 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
```bash
# 获取memory中的文档目录
DOC_DIR=$(find ~/.sa/memory/ -type d -name "docs")
```
...
执行以下脚本逻辑来获取变量 $SA_ROOT：

```bash
# 获取二进制绝对路径
COP_PATH=$(which sa 2>/dev/null || whereis sa | awk '{print $2}')

# 逻辑推导：如果路径包含 .sa/bin/sa，则根目录在其上三层
# 例如：/path/to/SolutionArchitect/.sa/bin/sa -> /path/to/SolutionArchitect
if [[ "$COP_PATH" == *".sa/bin/sa" ]]; then
    SA_ROOT=$(echo "$COP_PATH" | sed 's/\/\.sa\/bin\/sa//')
else
    # 兜底：尝试获取所在目录的父目录
    SA_ROOT=$(dirname $(dirname "$COP_PATH") 2>/dev/null || echo ".")
fi

echo "Detected SolutionArchitect Root: $SA_ROOT"
```
...
if [ -d "$DOC_DIR" ]; then
    ...
    # 如果推导路径不对，执行全局模糊搜索
    find "$SA_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```
**如果项目文档不存在，搜索工作目录**

如果还是找不到文档，搜索 sa 安装路径下的可用文档内容：

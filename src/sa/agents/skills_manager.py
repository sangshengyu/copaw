...
def _extract_requirements(post: dict[str, Any]) -> SkillRequirements:
    ...
    if "openclaw" in metadata:
        ...
        requires = metadata["sa"].get("requires", {})
    else:
...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"SOLUTION_ARCHITECT_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    ``SOLUTION_ARCHITECT_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="sa_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"sa_skill_stage_{skill_name}_"),
    )

# -*- coding: utf-8 -*-
"""SolutionArchitect Agents Module.
...
Public API:
- SAAgent: Main agent class
- create_model_and_formatter: Factory for creating models and formatters

Example:
    >>> from sa.agents import SAAgent, create_model_and_formatter
    >>> agent = SAAgent()
    >>> # Or with custom model
    >>> model, formatter = create_model_and_formatter()
"""

# SAAgent is lazy-loaded so that importing agents.skills_manager (e.g.
# from CLI init_cmd/skills_cmd) does not pull react_agent, agentscope, tools.
# pylint: disable=undefined-all-variable
__all__ = ["SAAgent", "create_model_and_formatter"]


def __getattr__(name: str):
    """Lazy load heavy imports."""
    if name == "SAAgent":
        from .react_agent import SAAgent

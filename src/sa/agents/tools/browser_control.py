...
_USE_SYNC_PLAYWRIGHT = (
    sys.platform == "win32" and os.environ.get("SOLUTION_ARCHITECT_RELOAD_MODE") == "1"
)
...
def _ensure_playwright_async():
    ...
    try:
        ...
        raise ImportError(
            "Playwright not installed. Use the same Python that runs SolutionArchitect (e.g. "
...
def _sync_browser_launch(state: dict, cdp_port: int = 0):
    ...
    use_default = not is_running_in_container() and os.environ.get(
        "SOLUTION_ARCHITECT_BROWSER_USE_DEFAULT",
        "1",

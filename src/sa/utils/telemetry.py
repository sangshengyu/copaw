...
TELEMETRY_ENDPOINT = "https://sa-telemetry-xissagieap.cn-hangzhou.fcapp.run"
...
def _detect_install_method() -> str:
    """Detect how SolutionArchitect was installed based on environment signals."""
    import os

    if os.environ.get("SOLUTION_ARCHITECT_RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("SOLUTION_ARCHITECT_DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - sa_version: SolutionArchitect version string
    - install_method: How SolutionArchitect was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running sa (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as sa_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "sa_version": _safe_get(lambda: sa_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current SolutionArchitect version string."""
    try:
        from ..__version__ import __version__ as sa_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when SolutionArchitect is upgraded (or downgraded) to a version
    that hasn't been collected before.

    Args:
        working_dir: Path to SolutionArchitect working directory
    ...
    try:
        ...
        # v1.1 compat: single sa_version field
        return marker_data.get("sa_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("sa_version", "")
                    if old_ver:

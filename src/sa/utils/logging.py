# -*- coding: utf-8 -*-
"""Logging setup for SolutionArchitect: console output and optional file handler."""
import io
import logging
...
_SA_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_SA_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (sa), not deps."""
    ...
    # Only attach handler to our namespace so only sa.* logs are printed.
...
def add_sa_file_handler(log_path: Path) -> None:
    """Add a file handler to the sa logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "sa.log").
    """
    ...
    if is_windows_or_linux:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path,
            encoding="utf-8",
            maxBytes=_SA_LOG_MAX_BYTES,
            backupCount=_SA_LOG_BACKUP_COUNT,
        )

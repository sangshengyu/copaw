...
from .process_utils import (
    ...
    _is_sa_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/sa/json"
...
class InstallInfo:
    """Information about the current SolutionArchitect installation."""
...
class RunningServiceInfo:
    """Detected SolutionArchitect service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published SolutionArchitect version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest SolutionArchitect version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest SolutionArchitect version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest SolutionArchitect version.",
        )
    return version
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running SolutionArchitect HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local SolutionArchitect service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_sa_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a SolutionArchitect HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running SolutionArchitect service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"SolutionArchitect service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `sa shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING SA SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `sa shutdown` will forcibly terminate the current "
        "SolutionArchitect backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `sa shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `sa shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "sa"]
    ...
    click.echo("")
    click.echo("Running `sa shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `sa shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`sa shutdown` failed. Please stop the running SolutionArchitect "
            "service manually before running `sa update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"sa=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from sa.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[sa] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[sa] Updating SolutionArchitect "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[sa] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[sa] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[sa] Update completed successfully.")
        click.echo(
            "[sa] Please restart any running SolutionArchitect service "
            "to use the new version.",
        )
    else:
        click.echo(f"[sa] Update failed with exit code {return_code}.")
        click.echo(
            "[sa] Please fix the error above and run "
            "`sa update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade SolutionArchitect in the current Python environment."""
    ...
    if version_check is False:
        click.echo("SolutionArchitect is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `sa update`, or rerun "
                "without `--yes` to confirm a forced `sa shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                "Detected "
                f"{_running_service_display(running)} after `sa shutdown`. "
                "Please stop it manually before running `sa update`.",
            )

    if not yes and not click.confirm(
        f"Update SolutionArchitect to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return

...
def _matches_sa_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a SolutionArchitect CLI invocation."""
...
def _is_sa_service_command(command: str) -> bool:
    """Return whether the command line looks like a local SolutionArchitect app."""
    return _matches_sa_cli_command(command, "app")


def _is_sa_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a SolutionArchitect CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "sa" or _matches_sa_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local SolutionArchitect service."""
    if not host:
        return []

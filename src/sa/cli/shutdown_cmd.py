...
def _find_desktop_wrapper_pids() -> set[int]:
    """Find `sa desktop` wrapper processes for this project."""
    matches: set[int] = set()
    patterns = (
        " -m sa desktop",
        " sa desktop",
...
def _find_windows_wrapper_ancestor_pids(pids: set[int]) -> set[int]:
    """Find SolutionArchitect wrapper/supervisor ancestors for Windows backend PIDs."""
    ...
    for pid in pids:
        ...
        while True:
            ...
            if _is_sa_wrapper_process(parent_info[1], parent_info[2]):
...
@click.command("shutdown", help="Force stop the running SolutionArchitect app processes.")
@click.option(
    "--port",
    default=None,
    type=int,
...
def shutdown_cmd(ctx: click.Context, port: Optional[int]) -> None:
    """Stop the running SolutionArchitect app processes.

    `sa app` only starts the backend process. The web console is normally
    ...
    if not all_targets:
        raise click.ClickException(
            "No running SolutionArchitect backend/frontend process was found.",
        )
    ...
    if stopped:
        click.echo(
            "Stopped SolutionArchitect processes: "

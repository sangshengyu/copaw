# -*- coding: utf-8 -*-
"""sa uninstall — remove the SolutionArchitect environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove SolutionArchitect PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# SolutionArchitect" comment line and the export PATH line
    cleaned = re.sub(
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove SolutionArchitect environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL SolutionArchitect data in {wd}")
    else:
        click.echo(
            "This will remove the SolutionArchitect Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("SolutionArchitect uninstalled. Please restart your terminal.")

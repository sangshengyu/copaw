...
def chats_group() -> None:
    ...
    \b
    Common examples:
      sa chats list                    # List all chats
      sa chats list --user-id alice    # Filter by user
      sa chats get <chat_id>           # View details
      sa chats create --session-id s1 --user-id u1
      sa chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      sa chats list
      sa chats list --user-id alice
      sa chats list --channel discord
      sa chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `sa chats list`.

    \b
    Examples:
      sa chats get 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
...
def create_chat(
    ...
    Inline creation examples:
      sa chats create --session-id "discord:alice" \\
...
def update_chat(
    ...
    \b
    Examples:
      sa chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
    payload = {"name": name}
...
def delete_chat(
    ...
    \b
    Examples:
      sa chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}

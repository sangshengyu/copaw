...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.sa-bound paths to current WORKING_DIR.

    This keeps SOLUTION_ARCHITECT_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.sa/media" or
    "/Users/x/.sa/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.sa"
...
def get_available_channels() -> Tuple[str, ...]:
    """Return channel keys enabled for this run.

    Since the channels module has been removed, this function now returns
    a default tuple with only "console" as the available channel.

    Environment variables are still respected for backward compatibility:
    * SOLUTION_ARCHITECT_ENABLED_CHANNELS — whitelist (only these channels are active).
    * SOLUTION_ARCHITECT_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, SOLUTION_ARCHITECT_ENABLED_CHANNELS takes precedence.
    * If neither is set, returns ("console",).
    """
    # Default channels since channels module was removed
    default_channels = ("console",)

    raw_enabled = os.environ.get("SOLUTION_ARCHITECT_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
        enabled = {ch.strip() for ch in raw_enabled.split(",") if ch.strip()}
        return (
            tuple(k for k in default_channels if k in enabled)
            or default_channels
        )

    raw_disabled = os.environ.get("SOLUTION_ARCHITECT_DISABLED_CHANNELS", "").strip()
    if raw_disabled:
        disabled = {ch.strip() for ch in raw_disabled.split(",") if ch.strip()}
        return (
            tuple(k for k in default_channels if k not in disabled)
            or default_channels
        )
...
def is_running_in_container() -> bool:
    ...
    Prefer env SOLUTION_ARCHITECT_RUNNING_IN_CONTAINER (1/true/yes) at call time so

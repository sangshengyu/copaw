...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is SolutionArchitect. How can I help you?"
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps SOLUTION_ARCHITECT_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.sa/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    # When SOLUTION_ARCHITECT_WORKING_DIR is customized, historical data may still exist
    # under "~/.sa".
    old_workspace = Path("~/.sa").expanduser().resolve()

...
WORKING_DIR = (
    Path(EnvVarLoader.get_str("SOLUTION_ARCHITECT_WORKING_DIR", "~/.sa"))
    .expanduser()
    .resolve()
)
SECRET_DIR = (
...
JOBS_FILE = EnvVarLoader.get_str("SOLUTION_ARCHITECT_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("SOLUTION_ARCHITECT_CHATS_FILE", "chats.json")

# Builtin multi-agent profile: SolutionArchitect Q&A helper.
BUILTIN_QA_AGENT_ID = "SA_QA_Agent_0.1beta1"
...
TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "SOLUTION_ARCHITECT_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("SOLUTION_ARCHITECT_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("SOLUTION_ARCHITECT_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("SOLUTION_ARCHITECT_OPENAPI_DOCS", False)

# Memory directory
MEMORY_DIR = WORKING_DIR / "memory"

# Custom channel modules (installed via `sa channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "SOLUTION_ARCHITECT_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
MEMORY_COMPACT_RATIO = EnvVarLoader.get_float(
    "SOLUTION_ARCHITECT_MEMORY_COMPACT_RATIO",
    0.7,
    min_value=0,
    allow_inf=False,
)
...
# Example: SOLUTION_ARCHITECT_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("SOLUTION_ARCHITECT_CORS_ORIGINS", "").strip()

# LLM API retry configuration
LLM_MAX_RETRIES = EnvVarLoader.get_int(
    "SOLUTION_ARCHITECT_LLM_MAX_RETRIES",
    3,
    min_value=0,
)

LLM_BACKOFF_BASE = EnvVarLoader.get_float(
    "SOLUTION_ARCHITECT_LLM_BACKOFF_BASE",
    1.0,
    min_value=0.1,
)

LLM_BACKOFF_CAP = EnvVarLoader.get_float(
    "SOLUTION_ARCHITECT_LLM_BACKOFF_CAP",
    10.0,
    min_value=0.5,
)
...
LLM_MAX_CONCURRENT = EnvVarLoader.get_int(
    "SOLUTION_ARCHITECT_LLM_MAX_CONCURRENT",
    10,
    min_value=1,
)
...
LLM_MAX_QPM = EnvVarLoader.get_int(
    "SOLUTION_ARCHITECT_LLM_MAX_QPM",
    600,
    min_value=0,
)
...
LLM_RATE_LIMIT_PAUSE = EnvVarLoader.get_float(
    "SOLUTION_ARCHITECT_LLM_RATE_LIMIT_PAUSE",
    5.0,
    min_value=1.0,
)
...
LLM_RATE_LIMIT_JITTER = EnvVarLoader.get_float(
    "SOLUTION_ARCHITECT_LLM_RATE_LIMIT_JITTER",
    1.0,
    min_value=0.0,
)
...
LLM_ACQUIRE_TIMEOUT = EnvVarLoader.get_float(
    "SOLUTION_ARCHITECT_LLM_ACQUIRE_TIMEOUT",
    300.0,
    min_value=10.0,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            os.environ.get("SOLUTION_ARCHITECT_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "600"),
        ),
        1.0,
    )
except (TypeError, ValueError):

...
def _do_migrate_legacy_workspace() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default SolutionArchitect agent (migrated from legacy config)",
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.sa or WORKING_DIR)
...
def ensure_qa_agent_exists() -> None:
    ...
    ``sa_source_index``), and built-in tools are restricted (see
    ``build_qa_agent_tools_config``).
...
def _do_ensure_qa_agent() -> None:
    ...
    agent_config = AgentProfileConfig(
        ...
        description=(
            "Builtin Q&A helper for SolutionArchitect setup, local config under "
            "SOLUTION_ARCHITECT_WORKING_DIR, and documentation. Prefer reading files "

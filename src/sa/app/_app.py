...
from ..utils.logging import setup_logger, add_sa_file_handler
...
async def lifespan(
    ...
    add_sa_file_handler(WORKING_DIR / "sa.log")
...
# Console static dir: env, or sa package data (console), or cwd.
_CONSOLE_STATIC_ENV = "SOLUTION_ARCHITECT_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in sa package as static data
...
def read_root():
    ...
    return {
        "message": (
            "SolutionArchitect Web Console is not available. "
            "If you installed SolutionArchitect from source code, please run "
            "`npm ci && npm run build` in SolutionArchitect's `console/` "
            "directory, and restart SolutionArchitect to enable the "
            "web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current SolutionArchitect version."""
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get("/sa-symbol.svg")
    def _console_icon():
        f = _console_path / "sa-symbol.svg"
        if f.is_file():

...
Shared by in-chat /daemon <sub> and CLI `sa daemon <sub>`.
Logs: tail WORKING_DIR / "sa.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `sa app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {WORKING_DIR / 'sa.log'}"
    )


def run_daemon_logs(lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / sa.log."""
    log_path = WORKING_DIR / "sa.log"

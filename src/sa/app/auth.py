...
variable ``SOLUTION_ARCHITECT_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``SOLUTION_ARCHITECT_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = os.environ.get("SOLUTION_ARCHITECT_AUTH_ENABLED", "").strip().lower()
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``SOLUTION_ARCHITECT_AUTH_ENABLED``
    is truthy and both ``SOLUTION_ARCHITECT_AUTH_USERNAME`` and ``SOLUTION_ARCHITECT_AUTH_PASSWORD``
    ...
    username = os.environ.get("SOLUTION_ARCHITECT_AUTH_USERNAME", "").strip()
    password = os.environ.get("SOLUTION_ARCHITECT_AUTH_PASSWORD", "").strip()

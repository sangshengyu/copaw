...
info()  { printf "${GREEN}[sa]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[sa]${RESET} %s\n" "$*"; }
error() { printf "${RED}[sa]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
SA_HOME="${SA_HOME:-$HOME/.sa}"
SA_VENV="$SA_HOME/venv"
SA_BIN="$SA_HOME/bin"
PYTHON_VERSION="3.12"
SA_REPO="https://github.com/agentscope-ai/SolutionArchitect.git"
...
printf "${GREEN}[sa]${RESET} Installing SolutionArchitect into ${BOLD}%s${RESET}\n" "$SA_HOME"
...
uv venv "$SA_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$SA_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$SA_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/sa/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/sa/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/sa/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing SolutionArchitect from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$SA_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing SolutionArchitect from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$SA_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$SA_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
    ...
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$SA_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package sa
fi

# Verify the CLI entry point exists
[ -x "$SA_VENV/bin/sa" ] || die "Installation failed: sa CLI not found in venv"
info "SolutionArchitect installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$SA_VENV/bin/python" -c "import importlib.resources, sa; p=importlib.resources.files('sa')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
chmod +x "$SA_BIN/sa"
info "Wrapper created at $SA_BIN/sa"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.sa/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.sa/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}SolutionArchitect installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$SA_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$SA_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}sa init${RESET}       # first-time setup\n"
printf "  ${BOLD}sa app${RESET}        # start SolutionArchitect\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}sa uninstall${RESET}\n"

# SolutionArchitect Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "llamacpp,mlx"] [-UvPath PATH]
#
# Installs SolutionArchitect into ~/.sa with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$SaHome     = if ($env:SA_HOME) { $env:SA_HOME } else { Join-Path $HOME ".sa" }
$SaVenv     = Join-Path $SaHome "venv"
$SaBin      = Join-Path $SaHome "bin"
$PythonVersion = "3.12"
$SaRepo     = "https://github.com/agentscope-ai/SolutionArchitect.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[sa] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[sa] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[sa] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
SolutionArchitect Installer for Windows
...
Environment:
  SA_HOME            Installation directory (default: ~/.sa)
"@
    exit 0
}

Write-Host "[sa] " -ForegroundColor Green -NoNewline
Write-Host "Installing SolutionArchitect into " -NoNewline
Write-Host "$SaHome" -ForegroundColor White
...
uv venv $SaVenv --python $PythonVersion --quiet --clear
if ($LASTEXITCODE -ne 0) { Stop-WithError "Failed to create virtual environment" }

$VenvPython = Join-Path $SaVenv "Scripts\python.exe"
...
# ── Step 3: Install SolutionArchitect ────────────────────────────────────────────────────
$ExtrasSuffix = ""
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\sa\console"
...
$VenvSa = Join-Path $SaVenv "Scripts\sa.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing SolutionArchitect from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\SolutionArchitect"
        }
        Write-Info "Installing SolutionArchitect from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "sa-install-$(Get-Random)"
        try {
            git clone --depth 1 $SaRepo $cloneDir
    ...
    $package = "sa"
    if ($Version) { $package = "sa==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package sa
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvSa)) { Stop-WithError "Installation failed: sa CLI not found in venv" }

Write-Info "SolutionArchitect installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, sa; p=importlib.resources.files('sa')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $SaBin -Force | Out-Null

$wrapperPath = Join-Path $SaBin "sa.ps1"
$wrapperContent = @'
# SolutionArchitect CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$SaHome = if ($env:SA_HOME) { $env:SA_HOME } else { Join-Path $HOME ".sa" }
$RealBin   = Join-Path $SaHome "venv\Scripts\sa.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "SolutionArchitect environment not found at $SaHome\venv"
...
$cmdWrapperPath = Join-Path $SaBin "sa.cmd"
$cmdWrapperContent = @"
@echo off
REM SolutionArchitect CLI wrapper — delegates to the uv-managed environment.
set "SA_HOME=%SA_HOME%"
if "%SA_HOME%"=="" set "SA_HOME=%USERPROFILE%\.sa"
set "REAL_BIN=%SA_HOME%\venv\Scripts\sa.exe"
if not exist "%REAL_BIN%" (
    echo Error: SolutionArchitect environment not found at %SA_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use SolutionArchitect."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "SolutionArchitect installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$SaHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  sa init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  sa app"  -ForegroundColor White -NoNewline; Write-Host "        # start SolutionArchitect"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "sa uninstall" -ForegroundColor White

...
$Archive = Join-Path $Dist "sa-env.zip"
$Unpacked = Join-Path $Dist "win-unpacked"
$NsiPath = Join-Path $PackDir "sa_desktop.nsi"
...
$VersionFile = Join-Path $RepoRoot "src\sa\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "sa-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "sa-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "SolutionArchitect Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var SOLUTION_ARCHITECT_LOG_LEVEL or default to "info"
if not defined SOLUTION_ARCHITECT_LOG_LEVEL set "SOLUTION_ARCHITECT_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\sa_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.sa\config.json" (
  "%~dp0python.exe" -u -m sa init --defaults --accept-security
)
"%~dp0python.exe" -u -m sa desktop --log-level %SOLUTION_ARCHITECT_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "SolutionArchitect Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with SOLUTION_ARCHITECT_LOG_LEVEL)
if not defined SOLUTION_ARCHITECT_LOG_LEVEL set "SOLUTION_ARCHITECT_LOG_LEVEL=debug"
...
echo ====================================
echo SolutionArchitect Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo Log Level: %SOLUTION_ARCHITECT_LOG_LEVEL%
...
echo [Launch] Starting SolutionArchitect Desktop with log-level=%SOLUTION_ARCHITECT_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m sa desktop --log-level %SOLUTION_ARCHITECT_LOG_LEVEL%
echo.
echo [Exit] SolutionArchitect Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "SolutionArchitect Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\SolutionArchitect Desktop.bat"
WshShell.Run Chr(34) & batPath & Chr(34), 0, False
Set WshShell = Nothing
"@ | Set-Content -Path $LauncherVbs -Encoding ASCII

# Create sa.cmd wrapper in env root so "sa" resolves to this
# instead of Scripts\sa.exe whose embedded Python path may be stale
# after conda-pack/unpack.
$SaCmd = Join-Path $EnvRoot "sa.cmd"
@"
@"%~dp0python.exe" -u -m sa %*
"@ | Set-Content -Path $SaCmd -Encoding ASCII
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('sa'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] SA_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "SolutionArchitect-Setup-$Version.exe"
